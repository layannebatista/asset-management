...
export class ContextFilter {
  ...
  static metricsToChunks(metrics: CollectedMetrics): ContextChunk[] {
    ...
    if (jvm.heapUsagePct > 80) anomalyFlags.push('HIGH_HEAP_USAGE');
    if (jvm.gcPauseSeconds > 1) anomalyFlags.push('GC_PAUSE_ALERT');
    if (http.errorRatePct > 5) anomalyFlags.push('HIGH_ERROR_RATE');
    if (http.p95LatencyMs > 2000) anomalyFlags.push('HIGH_LATENCY');
    if (system.cpuUsagePct > 85) anomalyFlags.push('HIGH_CPU');
    if (jvm.liveThreads > 200) anomalyFlags.push('THREAD_SATURATION');
    ...
    return [
      {
        ...
        data: {
          ...
          avgDurationMs: Math.round(
            results.reduce((s, r) => s + r.duration, 0) / (results.length || 1),
          ),
        },
        baseRelevance: 0.9,
      },
  ...
  static workflowRunsToChunks(runs: GitHubWorkflowRun[]): ContextChunk[] {
    ...
    const avgDurationMs =
      completed.reduce((s, r) => s + (r.durationMs ?? 0), 0) / (completed.length || 1);
  ...
  private static classifyHeapPressure(pct: number): string {
  ...
  private static classifyLatency(p95Ms: number): string {
    if (p95Ms > 5000) return 'critical';
    if (p95Ms > 2000) return 'high';
    if (p95Ms > 500) return 'elevated';
    return 'normal';
  }

  private static classifyErrorRate(pct: number): string {
  ...
  private static extractSuite(result: AllureTestResult): string {
