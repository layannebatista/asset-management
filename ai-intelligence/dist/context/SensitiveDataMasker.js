...
exports.SensitiveDataMasker = void 0;
/**
...
class SensitiveDataMasker {
    ...
    static mask(input) {
        return SensitiveDataMasker.PATTERNS.reduce((text, { pattern, replacement }) => text.replace(pattern, replacement), input);
    }
     ...
     */
    static maskLogs(logs, maxLineLength = 300) {
        return logs
            .map((line) => SensitiveDataMasker.mask(line))
            .map((line) => (line.length > maxLineLength ? `${line.slice(0, maxLineLength)}…` : line));
    }
}
exports.SensitiveDataMasker = SensitiveDataMasker;
//# sourceMappingURL=SensitiveDataMasker.js.map