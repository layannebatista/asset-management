 ...
 *
 * <p>Este enum descreve SOMENTE estados possíveis. Regras de transição e validações ficam no
 * domínio (Asset).
 */
public enum AssetStatus {

  /** Ativo recém-cadastrado no sistema. Ainda não está em uso. */
  CADASTRADO(EnumSet.of(AssetAction.ATIVAR)),
  ...
  TRANSFERENCIA_SOLICITADA(
      EnumSet.of(AssetAction.APROVAR_TRANSFERENCIA, AssetAction.REJEITAR_TRANSFERENCIA)),

  /** Transferência aprovada e aguardando confirmação de recebimento. */
  TRANSFERENCIA_APROVADA(EnumSet.of(AssetAction.CONFIRMAR_RECEBIMENTO)),

  /** Ativo em manutenção. */
  EM_MANUTENCAO(EnumSet.of(AssetAction.RETORNAR_DA_MANUTENCAO)),

  /** Ativo em processo de inventário/conferência. */
  EM_INVENTARIO(EnumSet.of(AssetAction.CONFIRMAR_LOCALIZADO, AssetAction.MARCAR_NAO_LOCALIZADO)),

  /** Ativo não localizado durante inventário. */
  NAO_LOCALIZADO(EnumSet.of(AssetAction.LOCALIZAR_ATIVO, AssetAction.BAIXAR)),
