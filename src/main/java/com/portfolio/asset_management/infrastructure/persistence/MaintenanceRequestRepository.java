 ...
 *
 * <p>Responsável exclusivamente por persistência.
...
public interface MaintenanceRequestRepository
  ...
  /**
   * Retorna uma solicitação de manutenção ativa para o ativo,
   * se existir.
   *
   * <p>Usado para garantir que não existam duas
   * manutenções simultâneas para o mesmo ativo.
   */
  ...
  default Optional<MaintenanceRequest> findAtivaByAssetId(UUID assetId) {
    return findByAssetIdAndStatusIn(
        assetId,
        List.of(
            MaintenanceRequestStatus.CRIADA,
            MaintenanceRequestStatus.EM_MANUTENCAO));
  }
}
