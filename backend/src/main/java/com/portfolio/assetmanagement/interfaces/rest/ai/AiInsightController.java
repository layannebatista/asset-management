 ...
 *
 * <p>Access control: - ADMIN + GESTOR can trigger analyses (expensive LLM calls) - All
 * authenticated users can read history
 *
 * <p>The actual analysis runs in the ai-intelligence Node.js sidecar. This controller enforces JWT
 * auth + RBAC and proxies the call.
 */
...
public class AiInsightController {
  ...
  @Operation(
      summary = "Run observability analysis",
      description = "Analyzes JVM, HTTP, and system metrics for anomalies and bottlenecks")
  ...
  @Operation(
      summary = "Run test intelligence analysis",
      description = "Detects flaky tests, slow tests, and failure patterns from Allure results")
  ...
  @Operation(
      summary = "Run CI/CD pipeline analysis",
      description = "Analyzes GitHub Actions workflow performance and failure trends")
  ...
  @Operation(
      summary = "Run incident analysis",
      description = "Analyzes logs and errors to identify root causes and impacted system layers")
  ...
  @Operation(
      summary = "Run domain risk analysis",
      description = "Analyzes asset lifecycle data for inconsistencies and risk scenarios")
  ...
  @Operation(
      ...
      description =
          "Runs all 4 specialized agents (DevOps, QA, Backend, Architecture) concurrently")
  ...
  public ResponseEntity<JsonNode> multiAgent() {
    return respond(
        () ->
            service.analyze(
                "multi-agent",
                new AiAnalysisRequest(null, null, null, null, null, null, null, null, null)));
  }
  ...
  public ResponseEntity<JsonNode> history(
      @RequestParam(required = false) String type, @RequestParam(defaultValue = "20") int limit) {
    try {
