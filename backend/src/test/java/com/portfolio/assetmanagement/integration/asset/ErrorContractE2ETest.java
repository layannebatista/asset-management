...
class ErrorContractE2ETest extends BaseIntegrationTest {
  ...
  void resposta403NaoExpoeInformacoes() {
    ...
    // GESTOR tenta aposentar — endpoint exclusivo de ADMIN
    criarAtivo("ERR-RETIRE-001");
    MockMvcResponse listagem = apiClient.listarAtivos(token);
    // Pega o primeiro ativo da listagem para obter um ID válido
    Long assetId = ((Number) listagem.path("content[0].id")).longValue();
