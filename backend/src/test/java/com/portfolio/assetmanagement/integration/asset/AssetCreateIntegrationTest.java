...
class AssetCreateIntegrationTest extends BaseIntegrationTest {
  ...
  @DisplayName(
      "[INTEGRACAO][ASSET] ADMIN cria ativo com assetTag explícito — retorna 201 e campos corretos")
  void criaAtivoComAssetTagExplicito() {
    ...
    assertThat(((Number) response.path("organizationId")).longValue())
        .isEqualTo(organizacao.getId());
  ...
  @Story("Criação de ativo")
  @Severity(SeverityLevel.CRITICAL)
  @DisplayName("[INTEGRACAO][ASSET] GESTOR cria ativo na sua unidade — retorna 201")
  ...
  @DisplayName("[INTEGRACAO][ASSET] Criação com assetTag duplicado retorna 400")
  ...
  @Severity(SeverityLevel.NORMAL)
  @DisplayName(
      "[INTEGRACAO][ASSET] Body com campos em branco retorna 400 com detalhes dos campos inválidos")
  ...
  @Severity(SeverityLevel.CRITICAL)
  @DisplayName("[INTEGRACAO][ASSET] OPERADOR não pode criar ativo — retorna 403")
  void operadorNaoPodeCriarAtivo() {
    String token = loginComoOperador();

    MockMvcResponse response =
        apiClient.criarAtivo(
            organizacao.getId(), "ASSET-OP", AssetType.NOTEBOOK, "Modelo", unidade.getId(), token);
  ...
  @DisplayName("[INTEGRACAO][ASSET] Criação sem autenticação retorna 401")
