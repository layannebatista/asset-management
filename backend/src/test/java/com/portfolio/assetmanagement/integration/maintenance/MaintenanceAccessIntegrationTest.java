...
class MaintenanceAccessIntegrationTest extends BaseIntegrationTest {
  ...
  @DisplayName(
      "[INTEGRACAO][ASSET] MA01 - GESTOR vê apenas manutenções da sua unidade — sem acesso à manutenção de outra unidade")
  void ma01GestorNaoVeManutencaoDeOutraUnidade() {
    ...
    var adminOutra =
        testDataHelper.criarAdmin("admin2@test.com", "Senha@123", outraOrg, outraUnidade);
    ...
    apiClient.criarManutencao(
        ativoPrincipal.getId(), "Manutenção da organização principal", tokenAdmin);
  ...
  @DisplayName("[INTEGRACAO][ASSET] MA02 - Listagem de manutenções sem autenticação — retorna 401")
  ...
  @Severity(SeverityLevel.CRITICAL)
  @DisplayName(
      "[INTEGRACAO][ASSET] MA03 - ADMIN pode criar, iniciar e concluir manutenção — fluxo completo 200")
  void ma03AdminFluxoCompletoComSucesso() {
    ...
    assertThat(apiClient.concluirManutencao(id, "HD substituído com sucesso", token).statusCode())
        .isEqualTo(200);
  }
  ...
  @Story("Controle de acesso")
  @Severity(SeverityLevel.CRITICAL)
  @DisplayName("[INTEGRACAO][ASSET] MA04 - GESTOR pode criar e cancelar manutenção")
  ...
  @Severity(SeverityLevel.CRITICAL)
  @DisplayName(
      "[INTEGRACAO][ASSET] MA05 - GESTOR não pode criar manutenção para ativo de outra unidade — retorna 403")
  void ma05GestorNaoPodeCriarManutencaoEmOutraUnidade() {
    ...
    MockMvcResponse response =
        apiClient.criarManutencao(
            ativoOutraUnidade.getId(), "Tentativa em unidade alheia", tokenGestor);
